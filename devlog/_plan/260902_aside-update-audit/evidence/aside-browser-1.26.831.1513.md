...
'}));async function writeAsideBrowserSkill(Cn){let ei=getAsideBrowserSkillFile(Cn);return await mkdir(path.dirname(ei),{recursive:!0}),await writeFile(ei,ASIDE_BROWSER_SKILL,{encoding:`utf-8`,flag:`w`}),{path:ei}}var developersRouter,init_routes$17=__esmMin((()=>{init_zod(),init_trpc$1(),init_agent_installations(),init_cli_installation(),init_skill(),developersRouter=router({getAsideBrowserSkill:publicProcedure.query(()=>({filename:`SKILL.md`,content:ASIDE_BROWSER_SKILL})),checkCliInstallation:publicProcedure.query(async()=>getCliInstallationStatus()),installCli:publicProcedure.mutation(async()=>(await installCli(),getCliInstallationStatus())),checkAgentInstallations:publicProcedure.query(()=>({agents:listAgentInstallations(),general:getGeneralSkillInstallation()})),installAsideBrowserSkill:publicProcedure.input(object({id:developerSkillTargetIdSchema})).mutation(async({input:Cn})=>Cn.id===`general`?{paths:(await Promise.all(listInstalledSkillTargetDirs().map(writeAsideBrowserSkill))).map(Cn=>Cn.path)}:{paths:[(await writeAsideBrowserSkill(getSkillTargetDir(Cn.id))).path]})})}));function ingestInboxMessage(Cn,ei){let ti=nanoid(),ni=new Date().toISOString(),ri={id:ti,...ei,receivedAt:ni,deliveryState:`pending`},ii=getWebPushNotificationOrigin(ei);if(ii&&getNotificationGrantRecord(Cn,ii)?.revokedAt)return{message:ri,inserted:!1,matchedRoutines:[]};let ai={source:ei.source,externalConversationId:ei.externalConversationId,from:ii??ei.from?.id??ei.from?.name,threadKey:ei.threadKey,title:ei.title,content:ei.content,metadata:ei.metadata},oi=stateDb(Cn),si=oi.transaction(ii=>{let oi=listMatchingEventRoutines(Cn,ai,new Date);if(![`web-push-notification`,`agent-to-agent`,`webhook`].includes(ei.source)&&oi.length===0)return{message:ri,inserted:!1,matchedRoutines:oi};let si=ei.externalEventId?ii.select().from(InboxMessageTable).where(and(eq(InboxMessageTable.source,ei.source),eq(InboxMessageTable.externalEventId,ei.externalEventId),...ei.metadata.connectionId?[ei.externalConversationId?eq(InboxMessageTable.externalConversationId,ei.externalConversationId):isNull(InboxMessageTable.externalConversationId),sql`json_extract(${InboxMessageTable.metadata}, '$.connectionId