...
## 부록 — `check-wp*.ps1` 은 이 목표 것이 아니다

같은 플랜 유닛 안에 있는 `check-wp1.ps1` ~ `check-wp5.ps1` 은 **앞선 목표**
(듀얼셸 Windows 커버리지) 의 체크다. 그 목표의 wp 번호와 이 함대 합류 목표의 wp 번호가
우연히 겹칠 뿐, 서로 무관하다.

지금 `check-wp1.ps1` 을 돌리면 3건이 FAIL 인데 전부 정상이다.
SKILL.md 줄 수 베이스라인(499 → 495)은 그 뒤 라우팅 작업에서 바뀐 값이고,
"wp2 에서 만들 파일" 두 개는 그 목표가 끝나면서 이미 만들어졌다. 과거 시점에 고정된
단언이라 지금은 참이 아니다.

이 목표의 wp1 체크는 `aside-memory-sync/tests/merge-driver-probe.py` 와
위 2·4절의 바이트 측정이다. 체크 영수증도 그걸로 끊었다.
