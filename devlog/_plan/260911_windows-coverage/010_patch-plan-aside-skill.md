...
**예산 정정 (wp2 A 감사).** D2 의 55줄은 *총합* 이고 **순삭제는 50줄** 이다.
L302(1줄)와 L335-338(4줄)은 삭제가 아니라 제자리 재작성이라 빼면 안 된다.
따라서 목표는 468 이 아니라 `499 - 50 + 24 = 473` 이다. 500 까지 27줄 여유.
468 을 목표로 잡고 억지로 더 깎지 않는다.

**이음매 (자르면 문장이 깨지는 곳).** 잘라내기 전에 아래를 같이 손본다.

| 위치 | 문제 | 조치 |
|---|---|---|
| L3 frontmatter | `on macOS` 가 남는다. AC1 정규식이 못 잡는다 | 양 플랫폼 문구로 교체 |
| L62 | 콜론이 L64-66 코드를 도입한다 | 콜론 문장을 호스트 레이어 포인터로 바꾼다 |
| L86 | 콜론이 L88-90 을 도입한다 | 같음 |
| L245 `Assembled:` | L247-254 를 도입한다 | 한 줄 포인터로 대체 |
| L266-267 | "the Seatbelt profile" 의 유일한 선행사가 L54-55 다. 표에 없다 | L54-57 이관 시 이 두 줄도 같이 중립화. 안 하면 Seatbelt 가 SKILL.md 에 살아남는다 |
| L338-339 | 한 문장이 두 줄에 걸쳐 있다 | L335-338 만 자르면 L339 가 파편이 된다. L335-343 을 한 덩어리로 재작성 |
| L435-443 | google-sheets 예시는 갈 곳이 없다 | perl 없이 1~2줄로 남긴다. 통째로 지우면 유일한 예시가 사라진다 |
| L13-14 | GUI 앱 필요 + Remote Control 포인터는 호스트 파일에 자리가 없다 | 제자리 재작성으로 유지 |
